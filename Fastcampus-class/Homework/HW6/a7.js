...
function isEvenNumber(n) {
    // do something
    if (n%2 == 0) return true; // n을 2로 나눈 결과가 0이면 짝수이므로 true
    else return false; // 홀수인 경우 false
  }
  ...
  function Person(name, age) {
    // do something
    this.name = name; // input major to Person.name
    this.age = age; // input major to Person.age
  }
  ...
  */
  ...
  class FeStudent extends Student{
    // do something
    constructor(name, major){ // get name & major
        super(name); // inherit from Student.name
        this.major = major; // input major to FeStudent.major
    }
    study(){
  ...
  */
  ...
  function printTriangle(n) {
    ...
    /*loop*/
    for(let i = 0; i < n; i++){
        ans += "*"; // combine asterisks(str-type)
  ...
  */
  ...
  function printInvertedTriangle(n) {
      // do something
      let ans = ""; //init ans
      let blank = ""; // init blank
      /*reverse loop*/
      for(let i = n; i > 0; i--){
          if (i == n) {} // first loop, do noting
          else blank += " "; // next loops need blanks, combine blanks
          ans = "* ".repeat(i); // combine asterisk&blank(str-type)
          console.log(blank+ans); // print
      }
  }
